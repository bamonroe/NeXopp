...
| `textstroke` `{text,transform,text_style}` | `TextElement` | `font_family`/`font_size`/`color` map, and `font_weight` >= 600 / `font_style` `italic` cross over as the Pango `Bold`/`Italic` tokens on `font`; the affine's translation is (x, y). Defaults: `Sans`, 16 px = 12 pt, opaque black, origin |
