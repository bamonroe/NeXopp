...
| `textstroke` `{text,transform,text_style}` | `TextElement` | `font_family`/`font_size`/`color` map, and `font_weight` >= 600 / `font_style` `italic` cross over as the Pango `Bold`/`Italic` tokens on `font`; the affine's translation is (x, y). A `ranged_text_attributes` entry spanning the whole string ("select all, make it bold") is flattened into runs and, when uniform, `font_family`/`font_size`/`font_weight`/`font_style`/`text_color` **override** the base `text_style`; non-uniform ranges, `underline` and `strikethrough` are dropped and the box is never split. Defaults: `Sans`, 16 px = 12 pt, opaque black, origin |
